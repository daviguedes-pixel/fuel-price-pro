-- Script para mostrar TODAS as transações (negativadas e não negativadas)
-- Execute este script no Supabase SQL Editor

-- Primeiro, remover a função se já existir
DROP FUNCTION IF EXISTS public.get_descontos_indevidos(DATE, DATE);

-- Criar a função que mostra TODAS as transações
...
BEGIN
  ...
  resultado_final AS (
    SELECT 
      ...
      (tv.preco_calculado < cb.custo_dia) AS negativado
    FROM transacoes_validas tv
    LEFT JOIN custos_batch cb ON 
      cb.id_empresa = tv.id_empresa
      AND cb.denominacao_item = tv.denominacao_item
      AND cb.data_transacao = tv.data_transacao
    -- REMOVIDO: WHERE tv.preco_calculado < cb.custo_dia
    -- Agora mostra TODAS as transações
  )
  SELECT 
  ...
  ORDER BY 
    rf.negativado DESC,  -- Negativadas primeiro
    rf.data_transacao DESC, 
    rf.preco_calculado ASC;
END;
$$;

-- Comentário
COMMENT ON FUNCTION public.get_descontos_indevidos IS 
'Busca TODAS as transações do período com comparação de preço vs custo (não apenas negativadas)';
