...
export default function PriceRequest() {
      ...
      const enrichedData = (data || []).map((request: any) => {
        ...
        stationIds.forEach((stationId: string) => {
          if (stationId) {
            ...
            if (station) {
              stations.push({ name: station.nome_empresa || station.name, code: station.cnpj_cpf || station.id || station.id_empresa });
            }
          }
        });
  ...
  useEffect(() => {
    ...
    const fetchCostsForAllStations = async () => {
      if (!formData.station_ids || formData.station_ids.length === 0 || !formData.product) {
        setStationCosts({});
        return;
      }
      
      // Verificar se mudou
      const stationsKey = formData.station_ids.sort().join(',');
      if (stationsKey === lastSearchedStations.sort().join(',') && formData.product === lastSearchedProduct) {
        return;
      }
      
      setLastSearchedStations(formData.station_ids);
      setLastSearchedProduct(formData.product);
      
      const newStationCosts: Record<string, any> = {};
      
      // Buscar custos para cada posto
      for (const stationId of formData.station_ids) {
        const station = stations.find(s => s.id === stationId);
        if (!station) continue;
        
        const costs = await fetchCostForStation(stationId, station.name);
        if (costs) {
          newStationCosts[stationId] = costs;
        }
      }
      
      setStationCosts(newStationCosts);
      
      // Se houver apenas um posto, atualizar formData para compatibilidade
      if (formData.station_ids.length === 1 && newStationCosts[formData.station_ids[0]]) {
        const singleCost = newStationCosts[formData.station_ids[0]];
        setFormData(prev => ({
          ...prev,
          purchase_cost: singleCost.purchase_cost.toFixed(4),
          freight_cost: singleCost.freight_cost.toFixed(4)
        }));
      }
    };
    
    fetchCostsForAllStations();
  }, [formData.station_ids, formData.product, stations]);
  ...
  const calculateMargin = useCallback(() => {
    try {
      ...
      // Se houver múltiplos postos, calcular para cada um
      if (formData.station_ids && formData.station_ids.length > 1) {
        const updatedCosts: Record<string, any> = {};
        
        for (const stationId of formData.station_ids) {
          const stationCost = stationCosts[stationId];
          if (!stationCost) continue;
          
          const purchaseCost = stationCost.purchase_cost || 0;
          const freightCost = stationCost.freight_cost || 0;
          ...
          const marginCents = !isNaN(suggestedPrice) && suggestedPrice > 0 
            ? Math.round((suggestedPrice - finalCost) * 100) 
            : 0;
          
          updatedCosts[stationId] = {
            ...stationCost,
        ...
        setStationCosts(updatedCosts);
        
        // Calcular média para exibição geral
        const margins = Object.values(updatedCosts).map((c: any) => c.margin_cents || 0);
        const avgMargin = margins.length > 0 ? Math.round(margins.reduce((a, b) => a + b, 0) / margins.length) : 0;
        setMargin(avgMargin);
        setCalculatedPrice(suggestedPrice);
      } else {
        // Cálculo único (compatibilidade com código antigo)
  ...
  }, [formData.suggested_price, formData.current_price, formData.purchase_cost, formData.freight_cost, formData.payment_method_id, formData.station_ids, stationCosts, paymentMethods, stationPaymentMethods]);
    ...
    try {
      ...
      if (formData.station_ids && formData.station_ids.length > 1) {
        ...
        for (const stationId of formData.station_ids) {
          ...
          // ARLA compensation (se aplicável)
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    if (!isDraft && ((!formData.station_ids || formData.station_ids.length === 0) || !formData.client_id || !formData.product || !formData.suggested_price)) {
    ...
    try {
      ...
      // Usar station_ids (array) se disponível, senão usar station_id (compatibilidade)
      const stationIdsToSave = formData.station_ids && formData.station_ids.length > 0
        ? formData.station_ids
        : (formData.station_id && formData.station_id !== 'none' ? [formData.station_id] : []);
      
      // Se houver múltiplos postos, criar uma solicitação para cada um
      const stationsToProcess = stationIdsToSave.length > 1 ? stationIdsToSave : [stationIdsToSave[0] || formData.station_id].filter(Boolean);
      
      const stationIdToSave = stationsToProcess.length > 0 ? stationsToProcess[0] : null; // Primeiro para compatibilidade
      ...
      const requestData = {
        station_id: stationIdToSave,
        station_ids: stationIdsToSave.length > 0 ? stationIdsToSave : null, // Array JSON
        client_id: clientIdToSave,
      ...
      console.log('📝 Postos para processar:', stationsToProcess.length > 1 ? `${stationsToProcess.length} postos` : '1 posto');
      ...
      if (stationsToProcess.length > 1) {
        const insertPromises = stationsToProcess.map(async (stationId) => {
          ...
          const stationRequestData = {
            ...
            station_ids: [stationId], // Array com apenas este posto
        ...
        if (errors.length > 0) {
          console.error('❌ Erros ao criar solicitações:', errors);
          ...
          if (firstError.code === '42501' || firstError.message?.includes('permission') || firstError.message?.includes('policy')) {
            ...
            toast.error(`Erro ao salvar ${errors.length} de ${stationsToProcess.length} solicitações: ${firstError.message || 'Erro desconhecido'}`);
          }
          
          if (successes.length > 0) {
            toast.success(`${successes.length} solicitação(ões) criada(s) com sucesso!`);
          }
        ...
        // Todas as solicitações foram criadas com sucesso
        const data = successes[0]?.data; // Usar a primeira para exibir na tela de sucesso
        toast.success(`${successes.length} solicitação(ões) criada(s) com sucesso!`);
        
        // Continuar com o fluxo normal usando a primeira solicitação criada
        if (!data) {
          setLoading(false);
          return;
        }
        
        // Atualizar savedSuggestion com dados da primeira solicitação
        const firstStation = stations.find(s => s.id === stationsToProcess[0]);
        const savedSuggestionData = {
          ...data,
          stations_list: stationsToProcess.map(sid => {
            const st = stations.find(s => s.id === sid);
            return st ? { id: st.id, name: st.name, code: st.code } : null;
          }).filter(Boolean)
        };
        
        setSavedSuggestion(savedSuggestionData);
      ...
      // Código original para um único posto
      const { data, error } = await supabase
        .from('price_suggestions')
        .insert([requestData])
        .select()
      ...
      <div className="container mx-auto px-4 py-4 space-y-4">
        ...
        <div className="lg:col-span-2">
                ...
                {/* Postos (múltiplos) */}
                  <div className="md:col-span-2">
                    <MultiStationSelector
                      ...
                      onSelect={(stationIds) => {
                        handleInputChange("station_ids", stationIds);
                        // Manter station_id como primeiro para compatibilidade
                        if (stationIds.length > 0) {
                          handleInputChange("station_id", stationIds[0]);
                        } else {
                          handleInputChange("station_id", "");
                        }
                      }}
                      required={true}
                    />
            ...
            <div className="space-y-6">
              ...
              {/* Se houver múltiplos postos, mostrar custos separados */}
              {formData.station_ids && formData.station_ids.length > 1 ? (
                <div className="space-y-4">
                  {formData.station_ids.map((stationId) => {
                    ...
                    return (
                      <Card key={stationId} className="border-2">
                        <CardHeader>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <Input
                                type="text"
                                value={stationCost.purchase_cost.toFixed(4)}
                                readOnly
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                type="text"
                                value={stationCost.freight_cost.toFixed(4)}
                                readOnly
                                className="bg-slate-100 dark:bg-slate-700 cursor-not-allowed"
                              />
                            </div>
                            {stationCost.final_cost !== undefined && (
                              <>
                                <div className="space-y-2">
                                  <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                    Custo Final (R$/L)
                                  ...
                                  <Input
                                    type="text"
                                    value={stationCost.final_cost.toFixed(4)}
                                    readOnly
                                    className="bg-slate-100 dark:bg-slate-700 cursor-not-allowed"
                                  />
                                </div>
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    type="text"
                                    value={`${stationCost.margin_cents || 0} centavos`}
                                    readOnly
                                    className={`bg-slate-100 dark:bg-slate-700 cursor-not-allowed ${
                                      (stationCost.margin_cents || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                                    }`}
                                  />
                                </div>
                                {stationCost.total_revenue !== undefined && formData.volume_projected && (
                                  <>
                                    <div className="space-y-2">
                                      <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                        Receita Total
                                      ...
                                      <Input
                                        type="text"
                                        value={formatPrice(stationCost.total_revenue)}
                                        readOnly
                                        className="bg-slate-100 dark:bg-slate-700 cursor-not-allowed"
                                      />
                                    </div>
                                    <div className="space-y-2">
                                      ...
                                      <Input
                                        type="text"
                                        value={formatPrice(stationCost.gross_profit || 0)}
                                        readOnly
                                        className={`bg-slate-100 dark:bg-slate-700 cursor-not-allowed ${
                                          (stationCost.gross_profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                                        }`}
                                      />
                                    </div>
                                  </>
            ...
            {/* Análise de Custos - para posto único (mantém compatibilidade) */}
            {(!formData.station_ids || formData.station_ids.length <= 1) && (costCalculations.finalCost > 0 || costCalculations.totalRevenue > 0) && (
